          ...
          <DrawerHeader className="relative">
            <DrawerClose asChild>
              <Button
                variant="ghost"
                size="icon"
                className="absolute right-4 top-4 w-9 h-9 rounded"
              >
                <X size={20} weight="bold" />
              </Button>
            </DrawerClose>
            <DrawerTitle className="sr-only">{want.title}</DrawerTitle>
          </DrawerHeader>
          {content}
        </DrawerContent>
      </Drawer>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[90vh] p-0 overflow-hidden">
        <DialogHeader className="sr-only">
          <DialogTitle>{want.title}</DialogTitle>
        </DialogHeader>
        <Button
          variant="ghost"
          size="icon"
          onClick={() => onOpenChange(false)}
          className="absolute top-4 right-4 z-50 bg-background/90 hover:bg-background w-9 h-9 rounded"
        >
          <X size={20} weight="bold" />
        </Button>
        {content}
